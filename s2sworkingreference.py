        ...
        try:
            async with websockets.connect(uri) as websocket:
                ...
                if all(state == "ready" for state in server_state.values()):
                    print(
                        "Both servers are ready for communication. You can now send messages."
                    )
                    while True:
                        message = await asyncio.get_event_loop().run_in_executor(
                            None, input, "Enter message to send: "
                        )
                        chat_message = {
                            "tag": "message",
...
async def connect_to_all_servers():
    tasks = [
        asyncio.create_task(connect_to_server(uri, server_id))
        for server_id, uri in server_mapping.items()
    ]
